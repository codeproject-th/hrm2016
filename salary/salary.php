...
if($_GET['salary_month']!='' and $_GET['salary_year']!=''){
	$where = " WHERE s.salary_month='".str_pad($_GET['salary_month'],2,"0",STR_PAD_LEFT)."' AND s.salary_year='".$_GET['salary_year']."'";
}

$sql="SELECT s.* , e.* FROM salary s LEFT JOIN employees e ON e.emp_id=s.employees_id ".$where;//ดึงข้อมูลเงินเดือน ตามเงื่อนไขเดือนที่เลือก
...
?>
		<tr>
            ...
            <td>
            <div class="btn-group">
            	<a href="index.php?m=<?=$m?>&p=salary_print&id=<?=$dbarr["salary_id"]?>"><button type="button" class="btn btn-xs btn-default"><i class="fa fa-fw fa-print"></i></button></a>
